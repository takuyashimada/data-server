...
describe("loadConfig", () => {
  it("allows the viewer MQTT URL to be overridden by environment", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "iot-data-server-"));
    const configDir = path.join(root, "config");
    await mkdir(configDir, { recursive: true });
...
`);
    ...
    vi.stubEnv("IOT_DATA_SERVER_VIEWER_MQTT_URL", "mqtt://receiver:1883");

    await expect(loadConfig(configDir)).resolves.toMatchObject({
      viewer: {
        realtime: {
          mqtt: {
            url: "mqtt://receiver:1883",
          },
        },
      },
    });
